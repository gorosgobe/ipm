...
for training in $training_list; do
  for repl in $replication_list; do
    ...
    echo "Starting training normal metastn, scale 0.5, for training data ${training_str} and replication ${repl}."
    time python3 train_meta_stn.py --name "final_meta_stn_040401_05_${dataset_str}_${training_str}_${repl}" --size 64\
    --epochs 200 --dataset "${dataset}" --split 0.4 0.4 0.1 --loc_lr 0.01 --model_lr 0.1 --batch_size 32 --scale 0.5 >> "$log_file"
    echo "Completed training."

    echo "Starting training DSAE metastn, scale 0.5 for training data ${training_str} and replication ${repl}."
    time python3 train_meta_stn.py --name "final_meta_stn_dsae_040401_05_${dataset_str}_${training_str}_${repl}" --size 64\
    ...
    echo "Starting training DSAE metastn with anneal, scale 0.5 for training data ${training_str} and replication ${repl}."
    time python3 train_meta_stn.py --name "final_meta_stn_dsae_anneal_040401_05_${dataset_str}_${training_str}_${repl}" --size 64\
    --epochs 200 --dataset "${dataset}" --split 0.4 0.4 0.1 --loc_lr 0.01 --model_lr 0.1 --batch_size 32 --scale 0.5\
