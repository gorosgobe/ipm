...
class CropSizeFeatureSearch(Saveable):
    ...
    def evaluation_function(self, parameterization):
        ...
        # not ideal, better to have a choice parameter, but then Sobol is used and can't plot, visualise x, y
        index = round(parameterization["feature"])
    ...
    def search(self, total_trials):
        best_parameters, values, experiment, model = optimize(
            parameters=[
                {"name": "feature", "type": "range", "bounds": [0.0, float(self.features - 1)]},
        ...
        self.improvement_plot = self.get_plot_str(improvement_plot)
